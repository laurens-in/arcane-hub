...
static const nrfx_spi_t can_spi = NRFX_SPI_INSTANCE(SPI_CAN_INSTANCE);  /**< SPI instance. */

/**@brief Function for handling MCP2515 INT Pin events.
 *
 * @param[in] 
 */ 
void mcp2515_int_pin_handler(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
...
void mcp_spi_init()
{
    ...
    nrfx_gpiote_in_config_t mcp2515_int_config = NRFX_GPIOTE_CONFIG_IN_SENSE_TOGGLE(true);
    mcp2515_int_config.pull = NRF_GPIO_PIN_PULLUP;

    err_code = nrfx_gpiote_in_init(MCP2515_PIN_INT, &mcp2515_int_config, mcp2515_int_pin_handler);
